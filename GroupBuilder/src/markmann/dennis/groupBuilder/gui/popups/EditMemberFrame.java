...
public final class EditMemberFrame extends JFrame implements DefaultFrame {
	...
	private EditMemberFrame(final Pojo pojo, final int rowID) {
		...
		this.pojo = pojo;
		BUILDER.setDefaultFrameSettings(this,
				"GroupBuilder - " + pojo.getTranslation("EditMember"));
		...
		final JTextField firstNameField = BUILDER.createTextField(this,
				"firstNameField", TEXT_FIELD_SIZE, 1, 1);
		final JTextField lastNameField = BUILDER.createTextField(this,
				"lastNameField", TEXT_FIELD_SIZE, 1, 2);
		final JTextField eMailField = BUILDER.createTextField(this,
				"eMailField", TEXT_FIELD_SIZE, 1, 3);
		final JComboBox<String> groupBox = BUILDER.createComboBox(this,
				"groupBox", pojo.getGroupListAsArray(), 1, 4);

		final JButton confirmationButton = BUILDER.createButton(this,
				"confirmationButton", pojo.getTranslation("Confirm"), 0, 5);
		final JButton abortButton = BUILDER.createButton(this, "abortButton",
				pojo.getTranslation("Abort"), 1, 5);
		...
		final EditMemberFrameListener listener = new EditMemberFrameListener(
				this, pojo, rowID, firstNameField, lastNameField, eMailField,
				groupBox);
	...
	private void fillFields(final Pojo pojo, final int rowID,
			final JTextField firstNameField, final JTextField lastNameField,
			final JTextField eMailField, final JComboBox<String> groupBox) {
		final Member member = pojo.getMemberList().get(rowID);
		firstNameField.setText(member.getFirstName());
		lastNameField.setText(member.getLastName());
		eMailField.setText(member.getEMailAdress());
		if (member.getGroup() != null) {
			groupBox.setSelectedIndex(this.getIndexOfEntry(
					pojo.getGroupListAsArray(), member.getGroup().getName()));
		}
	}
