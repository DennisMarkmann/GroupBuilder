...
/**
 * Listener for the settings frame. Used to fill the different values and to
 * close the window.
 * 
...
public class SettingsFrameListener implements ActionListener {
	...
	public SettingsFrameListener(final SettingsFrame settingsFrame,
			final Pojo pojo, final JTextField pathField,
			final JComboBox<String> languageBox, final JCheckBox archivingBox,
			final JTextField archiveField, final JTextField printerField,
	...
	public final void actionPerformed(final ActionEvent event) {
		...
		if (buttonClicked.getName().compareTo("pathButton") == 0) {
			try {
				this.settingsFrame.updatePathField(this.pathField,
						new PathChooser().changePath());
			...
			try {
				this.settingsFrame.updatePrinterField(this.printerField,
						new PrinterSelector().selectPrinter().getName());
			} catch (final java.lang.NullPointerException e) {
				new NotToHandleException();
			}
			...
			settings.setSendMailAutomatically(this
					.readCheckBox(this.sendMailsAutomatically));
			settings.setPrintAutomatically(this
					.readCheckBox(this.printOutAutomatically));
			settings.setLanguage(this.pojo.getLanguageList()[this.languageBox
					.getSelectedIndex()]);
			new LanguageChooser().chooseLanguage(this.pojo);
			settings.setArchivingDays(Integer.parseInt(this.archiveField
					.getText()));
			...
			OperationSuccessfullFrame.getInstance(
					this.pojo.getTranslation("SettingsChanged"), this.pojo);
