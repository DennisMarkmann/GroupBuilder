...
public class ConfirmationFrameListener implements ActionListener {
	...
	public ConfirmationFrameListener(final ConfirmationFrame confirmationFrame,
			final Pojo pojo, final String action, final Object object) {
		...
		outputOperations = Arrays.asList(pojo.getTranslation("Print"),
				pojo.getTranslation("SendMail"), pojo.getTranslation("Save"));
		memberOperations = Arrays.asList(pojo.getTranslation("AddMember"),
				pojo.getTranslation("EditMember"),
				pojo.getTranslation("RemoveMember"));
		groupOperations = Arrays.asList(pojo.getTranslation("AddGroup"),
	...
	private void useOutputOperation() {
		...
		if (this.action.equals(this.pojo.getTranslation("Print"))) {
			new PrintJobHelper().printOutForGroups(this.pojo,
					(ArrayList<Group>) this.object);
			this.message = this.pojo.getTranslation("PrintSuccess");

		} else if (this.action.equals(this.pojo.getTranslation("SendMail"))) {
			new EmailJobHelper().sendMailToGroups(this.pojo,
					(ArrayList<Group>) this.object);
	...
	private void useMemberOperation() {
		...
		if (this.action.equals(this.pojo.getTranslation("AddMember"))) {
			...
			final Member member = this.pojo.getMemberList().get(
					(int) (this.object));
	...
	private void useGroupOperation() {
		...
		if (this.action.equals(this.pojo.getTranslation("AddGroup"))) {
			...
			new GroupCreator(this.pojo).createGroup(group.getName(),
					group.getDescription(), group.getFixSize());
			...
			final Group group = this.pojo.getGroupList().get(
					(int) (this.object));

			new GroupCreator(this.pojo).removeGroup(group);
			this.generateGroupMessage(group, "Removed");

		} else if (this.action.equals(this.pojo
				.getTranslation("AutoCreateGroups"))) {
			new GroupCreator(this.pojo)
					.createGroupsAutmatically((int) this.object);
	...
	private void generateMemberMessage(final Member member,
			final String operation) {

		final String firstName = member.getFirstName();
		final String lastName = member.getLastName();
		this.message = this.pojo.getTranslation("Member") + " " + firstName
				+ " " + lastName + " "
				+ this.pojo.getTranslation("WasSuccessfully") + " "
	...
	private void generateGroupMessage(final Group group, final String operation) {
		...
		this.message = this.pojo.getTranslation("Group") + " " + groupName
				+ " " + this.pojo.getTranslation("WasSuccessfully") + " "
				+ this.pojo.getTranslation(operation) + ".";
	}
